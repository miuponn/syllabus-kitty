      ...
      {isOpen && (
        <div 
          className={`fixed z-50 transition-all duration-300 ease-out ${
            isExpanded 
              ? 'inset-4 sm:inset-8' 
              : 'bottom-6 left-6 w-[calc(100%-3rem)] sm:w-96 h-[500px] sm:h-[600px]'
          }`}
        >
          <div 
            ...
            <div className="px-4 py-2 flex gap-2 overflow-x-auto border-t border-purple-100 bg-white/50">
              {['📅 Deadlines', '🗓️ Schedule', '📋 Policies', '💡 Study Tips'].map((action) => (
                <button
                  ...
                  className="flex-shrink-0 px-3 py-1.5 text-xs rounded-full bg-purple-100 text-purple-700 hover:bg-purple-200 transition-colors whitespace-nowrap"
                >
                  {action}
                </button>
              ))}
